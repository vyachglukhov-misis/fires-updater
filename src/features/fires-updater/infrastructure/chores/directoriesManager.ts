...
import os from 'os';
import { PROJECT_ROOT_PATH } from '../../../../utils/constants.js'; // без алиасов потому что не могу запустить воркер :/
...
export const workingDirectories = {
  ...
  TILES_OUTPUT_DIR: path.join(PROJECT_ROOT_PATH, 'tiles'),

  // куда падают логи с каждой задачи
  UPDATING_TASKS_LOGS_DIR: path.join(PROJECT_ROOT_PATH, 'updating_logs'),

  // куда падают логи с запросов
  MAIN_LOGS_DIR: path.join(PROJECT_ROOT_PATH, 'logs'),
};
...
const directories = [
  ...
  { dirname: MAIN_LOGS_DIR, fileExtensions: [] },
  { dirname: UPDATING_TASKS_LOGS_DIR, fileExtensions: [] },
];
...
const deleteFilesFromDirectory = (
  dirPath: string,
  fileExtensionsToDelete: string[],
) => {
  try {
    const filesToDelete = fs
      .readdirSync(dirPath)
      .filter(fileName =>
        fileExtensionsToDelete.some(el => fileName.endsWith(el)),
      )
