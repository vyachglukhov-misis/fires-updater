...
export const writeDataToTiffPipeline = async (
  sectorData: SectorData,
  paramsMaxCoeff: Record<string, number>,
) => {
  try {
    ...
    const data = new Uint8Array(xSize * ySize).fill(0);

    const { gamma } = config.normalization;

    for (let i = 0; i < xSize * ySize; i++) {
      let coefficient = Object.entries(paramsData).reduce(
        (acc, [paramName, { paramData, weight }]) => {
      ...
      if (coefficient > 0) {
        ...
        data[i] = Math.round(coefficient * 255);
        pixelsAffected++;
      }
    }
