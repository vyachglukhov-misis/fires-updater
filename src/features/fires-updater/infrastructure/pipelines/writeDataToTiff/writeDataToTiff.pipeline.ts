...
export const writeDataToTiffPipeline = async (sectorData: SectorData, paramsMaxCoeff: Record<string, number>) => {
    try {
        ...
        for (let i = 0; i < xSize * ySize; i++) {
            let coefficient = Object.entries(paramsData).reduce((acc, [paramName, { paramData, weight }]) => {
                ...
                if (paramValue === undefined || paramMaxValue === undefined || paramMaxValue === 0) {
                    return acc
                }
